...
void runOnWindow(int W1, int H1, int W2, int H2, Mat inputImage, char *outName) {
	...
	double n = (double)rows * (double)cols;
	double k = 101.0;
	for (int i = 0; i < 101; i++)
	{
		if (i > 0)
		{
			...
			LLookupTable[i] = floor((((double)LPixelsum[i - 1] + (double)LPixelsum[i]) / 2.0) * (k / n));
		}
		else
		{
			LPixelsum[i] = LHistogram[i];
			LLookupTable[i] = floor(((double)LPixelsum[i] / 2.0) * (k / n));
		}
	}
